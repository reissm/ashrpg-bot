...
class ClassClient():
    ...
    def get_class_info(self, class_name):
        ...
        res = Embed(title=f"{class_name.capitalize()} - {cls['classType'].capitalize()} Class", color=ClassColors[class_name.upper()].value)
        res.add_field(name='Signature Ability', value=cls['signatureAbility'], inline=False)
        res.add_field(name='Hit Dice', value=cls['hitDice'], inline=False)
        res.add_field(name='Weapon Proficiencies', value=', '.join(cls['weaponProficiencies']), inline=False)
        res.add_field(name='Armor Proficiencies', value=', '.join(cls['armorProficiences']), inline=False)
    ...
    def search_talents_and_feats(self, class_name, query):
        ...
        talents = list(filter(lambda x : str(query).lower() in x.get('name', '').lower(), self.class_list[class_name]['feats']))

        if len(talents) == 1 or query.lower() in list(map(lambda x : x['name'].lower(), talents)):
            # Only a single talent found, so return all details about it
            talent = talents[0]

            res = Embed(title=f"{talent['name']} -- Level {talent['level']} {class_name.capitalize()} {talent['talentType'].capitalize()}", color=ClassColors(class_name.upper()).value)
            ...
            res.add_field(name='Requirements', value='\n'.join(talent['requirements']) if talent['requirements'] else 'None' , inline=False)
            res.add_field(name='Prerequisite', value=talent['requiredFeat'], inline=False)
        elif len(talents) >= 1:
            ...
            talent_map = {feat_name: [] for feat_name in list(map(lambda x : x['talentType'].capitalize(), talents))}
            for talent in list(map(lambda x : {'name': x['name'], 'type': x['talentType'].capitalize()}, talents)):
            ...
            res = Embed(title=f"Feature/ talent not found for class {class_name.capitalize()}: \"{query}\"", color=Color.red())
    ...
    def refresh_class_list(self):
        ...
        if response.status_code >= 400:
            raise RuntimeError(f"Unable to call AshRPG Class List API, reason: {response.json()}")
    ...
    def refresh_class_details(self, class_name):
        ...
        response = requests.get(url=f"{self.class_list_url}/{class_name}", headers=headers, verify=False)
        ...
        self.class_list[class_name]['feats'] = [*self.class_list[class_name]['features'], *self.class_list[class_name]['talents']]
